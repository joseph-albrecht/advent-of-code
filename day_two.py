...
def execute(pc, program):
    opcode = program[pc] 
    arg_1 = program[program[pc+1]]
    arg_2 = program[program[pc+2]]
    dest = program[pc+3]
    halt = False
    ...
    return not halt, program
def run_program(program):
    program_counter = 0
    run = True
    while(run):
        run, program = execute(program_counter, program)
        program_counter += 4

    return program[0]

def load_program():
    gravity_assist_program = list(map(int, open("day_two.input", 'r').read().split(',')))
    return gravity_assist_program


inputs = list(itertools.product(range(100), range(100)))
