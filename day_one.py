...
# module of mass 1969 is 654 + 216 + 70 + 21 + 5 = 966.  The fuel required by
# a module of mass 100756 and its fuel is: 33583 + 11192 + 3728 + 1240 + 411
# + 135 + 43 + 12 + 2 = 50346.  What is the sum of the fuel requirements for
# all of the modules on your spacecraft when also taking into account the
# mass of the added fuel? (Calculate the fuel requirements for each module
...
def fuel_calculation(mass):
    ...
    while(fuel_that_needs_fuel):
       fuel_for_fuel = fuel_calculation(fuel_that_needs_fuel)
...
parts_list = map(int, open("day_one.input", 'r').read().split()) 
parts_fuel_req = map(fuel_calculation, parts_list)
total_fuel_req = sum(parts_fuel_req)

print(f"total fuel requirement, {total_fuel_req}")
