...
# addition), the positions of the two inputs (9 and 10), and the position of
# the output (3). To handle this opcode, you first need to get the values at
# the input positions: position 9 contains 30, and position 10 contains
# 40. Add these numbers together to get 70. Then, store this value at the
# output position; here, the output position (3) is at position 3, so it
# overwrites itself. Afterward, the program looks like this:

# 1,9,10,70, 2,3,11,0, 99, 30,40,50 Step forward 4 positions to reach the
# next opcode, 2. This opcode works just like the previous, but it multiplies
...
def arg_value(pc, program, mode):
    if mode:
        return program[pc]
    else:
        return program[program[pc]]

def execute(pc, program):
    arg_3_mode, arg_2_mode, arg_1_mode, _, opcode = map(int, list(str(program[pc]).zfill(5)))

    inc = 0
    
    if opcode == 1:
        result = arg_value(pc+1, program, arg_1_mode) + \
                 arg_value(pc+2, program, arg_2_mode)
        ...
        print("in 3")
        program[program[pc+1]] = prog_input
        inc = 2
    elif opcode == 4:
        print(arg_value(pc+1, program, arg_1_mode))
        inc = 2
    elif opcode == 99:
        inc = 0
...
def run_program(program):
    ...
    run = True
    while(run):
        inc, program = execute(program_counter, program)
...
def load_program():
    gravity_assist_program = list(map(int, open("day_five.input", 'r').read().split(',')))
