...
def convert_relative_path_to_absolute(steps):
    absolute_nodes = [(0,0),]
    for step in steps:
        distance = abs(int(step[1:]))
        for n in range(distance):
            new_node = list(absolute_nodes[-1])
            if step[0] == 'U':
                new_node[1] += 1 
            ...
            absolute_nodes.append(tuple(new_node))

    return absolute_nodes

def calc_wire_distance(coordinates):
    return wire_one_nodes.index(coordinates) + wire_two_nodes.index(coordinates)
...
intersections = set(wire_one_nodes) & set(wire_two_nodes)
intersections.remove((0,0))

print(calc_wire_distance(min(intersections, key=calc_wire_distance)))
