...
#+begin_src python :results output
  ...
  for num, ingredient in enumerate(ingredients):
      if num == 0:
          recipe[ingredient] = 100
      else:
          recipe[ingredient] = 0

  def get_neighbor(a_recipe):
      ingredients_in_recipe = list(filter(lambda x: a_recipe[x] > 0, a_recipe))
      ingredient_to_decrease = random.choice(ingredients_in_recipe)

      ingredients_left = list(filter(lambda x: x != ingredient_to_decrease, ingredients.keys()))
      ingredient_to_increase = random.choice(ingredients_left)
    
      new_recipe = a_recipe.copy()
      new_recipe[ingredient_to_decrease] -= 1
      new_recipe[ingredient_to_increase] += 1
  ...
  def calc_score(r):
      values = {'capacity' : 0,
                ...
                'flavor' : 0,
                'texture' : 0}
      for ingredient, amount in r.items():
          for quality, value in ingredients[ingredient].items():
              if quality != "calories":
                  values[quality] += amount * value

      return functools.reduce(lambda x, y: x*y, values.values())
    
  for iteration in range(100000):
      neighbor = get_neighbor(recipe)
      neighbor_score = calc_score(neighbor)
      recipe_score = calc_score(recipe)

      if neighbor_score >= recipe_score:
          recipe = neighbor

  print(ingredients)
  print(recipe)
  print(calc_score(recipe))
...
#+begin_src python :results output
  ...
  def get_neighbor(a_recipe, loops=0):
      ingredients_in_recipe = list(filter(lambda x: a_recipe[x] > 0, a_recipe))
      ingredient_to_decrease = random.choice(ingredients_in_recipe)
      ...
      if new_recipe in tried_recipes and loops < 5:
          return get_neighbor(new_recipe, loops=loops+1)
      return new_recipe
...
#+begin_src python :results output
  data = open("inputs/2015-Day16", 'r').read().split("\n")
  ...
  for line in data:
      ...
      number = parts[1][:-1]
      one = parts[2][:-1]
      one_amount = parts[3][:-1]
      two = parts[4][:-1]
      two_amount = parts[5][:-1]
      three = parts[6][:-1]
      three_amount = parts[7]

      new_sue = {}
      new_sue[one] = one_amount
      new_sue[two] = two_amount
      new_sue[three] = three_amount

      sues[number] = new_sue

  for number, attributes in sues.items():
      if attributes.items() <= ticker_tape.items():
          print(number)
#+end_src
...
#+begin_src python :results output
  ...
  def compare(sue):
      for thing, number in sue.items():
          if thing == "cats":
              if sue["cats"] <= ticker_tape["cats"]:
                  return False
          elif thing == "trees":
              if sue["trees"] <= ticker_tape["trees"]:
                  return False
          elif thing == "pomeranians":
              if sue["pomeranians"] >= ticker_tape["pomeranians"]:
                  return False
          elif thing == "goldfish":
              if sue["goldfish"] >= ticker_tape["goldfish"]:
                  return False
          else:
              if sue[thing] != ticker_tape[thing]:
                  return False
...
#+begin_src python :results output
  voluems = list(map(lambda x: int(x), open("inputs/2015-Day17", 'r').read().split("\n")))
#+end_src
